...
Just send me your bill information to get started!"""
    ...
    async def _contains_bill_info(self, message: Message) -> bool:
        ...
        bill_keywords = ["bill", "amount", "total", "split", "pay", "₹", "$", "rs", "rupees"]
...
class BillExtractionHandler(BaseStepHandler):
        ...
        try:
            ...
            try:
                ...
                if state.context.get("attempt_count", 0) < 2:
                    questions = await self._generate_clarifying_questions(state, message)
                    ...
                    return StepResult(
                        response=Response(
                            content=self._get_fallback_instructions(message.message_type),
                            message_type=MessageType.TEXT
                        ),
                        context_updates={
    ...
    async def _handle_clarification_response(self, state: ConversationState, message: Message) -> StepResult:
        ...
        try:
            ...
            if "partial_bill_data" in state.context:
                bill_data = self._merge_bill_data(state.context["partial_bill_data"], bill_data)
            ...
            if validation_result.is_valid:
                ...
                questions = await self.bill_extractor.generate_clarifying_questions(bill_data)
                
                if questions:
                    question_text = "I still need some information:\n\n" + "\n".join(f"• {q}" for q in questions)
                else:
                    question_text = "Please provide the total bill amount and a brief description."
    ...
    async def _generate_clarifying_questions(self, state: ConversationState, message: Message) -> str:
        ...
        try:
            ...
            questions = await self.bill_extractor.generate_clarifying_questions(partial_data)
    ...
    def _format_validation_errors(self, validation_result: ValidationResult) -> str:
        ...
        for error in validation_result.errors:
            if "amount" in error.lower():
                ...
                error_messages.append("Please provide a description of what the bill is for")
            else:
    ...
    def _merge_bill_data(self, partial_data: Dict[str, Any], new_data: BillData) -> BillData:
        ...
        try:
            ...
            if is_confirmed:
                ...
                if state.context.get("validation_warnings"):
                    ...
                    if warnings:
                        confirmation_message += "⚠️ *Note:* " + "; ".join(warnings) + "\n\n"
                ...
                if "change" in error_message.lower():
                    ...
                    return StepResult(
                        ...
                        context_updates={
                            ...
                            "clarification_count": state.context.get("clarification_count", 0) + 1
                        }
                    )
            else:
                # Fallback response
...
class ContactCollectionHandler(BaseStepHandler):
    ...
    async def handle_message(self, state: ConversationState, message: Message) -> StepResult:
        ...
        try:
            ...
            if not state.context.get("contact_collection_started"):
                ...
                if missing_questions:
                    ...
                    return StepResult(
                        ...
                        context_updates={
                            ...
                            "processed_participants": [p.dict() for p in updated_participants]
                        }
                    )
                else:
                    ...
                    return StepResult(
                        ...
                        context_updates={
                            ...
                            "final_participants": [p.dict() for p in updated_participants]
                        }
                    )
                ...
                user_responses = self._parse_contact_responses(message.content, missing_questions)
                
                # Convert processed participants back to objects
                processed_participant_objects = [Participant(**p) for p in processed_participants]
                
                # Handle missing contacts with user responses
                final_participants, remaining_questions = await self.contact_manager.handle_missing_contacts(
                ...
                if remaining_questions:
                    ...
                    return StepResult(
                        ...
                        context_updates={
                            ...
                            "processed_participants": [p.dict() for p in final_participants]
                        }
                    )
                else:
                    ...
                    return StepResult(
                        ...
                        context_updates={
                            "contacts_collected": True,
                            "final_participants": [p.dict() for p in final_participants]
                        }
                    )
    ...
    def _parse_contact_responses(self, message_content: str, missing_questions: List[str]) -> Dict[str, str]:
...
class SplitCalculationHandler(BaseStepHandler):
    ...
    async def handle_message(self, state: ConversationState, message: Message) -> StepResult:
        ...
        try:
            ...
            participants_data = state.context.get("final_participants") or state.context.get("participants", [])
            ...
            if await self._is_custom_split_request(message):
                ...
                custom_amounts = await self.bill_splitter.parse_custom_amounts(message.content, participants)
                ...
                if custom_amounts:
                    ...
                    validation_result = await self.bill_splitter.validate_splits(bill_data, updated_participants)
                    
                    if validation_result.is_valid:
                        # Custom splits are valid
                        confirmation_display = await self.bill_splitter.format_split_confirmation(
                        ...
                        return StepResult(
                            ...
                            context_updates={
                                ...
                                "split_type": "custom",
                                "calculated_participants": [p.dict() for p in updated_participants],
                                "validation_warnings": validation_result.warnings
                            }
                        )
                    else:
                ...
                updated_participants = await self.bill_splitter.calculate_equal_splits(bill_data, participants)
                ...
                if validation_result.is_valid:
                    ...
                    logger.error(f"Equal split validation failed: {validation_result.errors}")
                    return StepResult(
                        response=Response(
    ...
    async def _is_custom_split_request(self, message: Message) -> bool:
        ...
        custom_indicators = [
            "₹", "rs", "rupees", "custom", "different", "change", "adjust",
            "more", "less", "should pay", "owes", ":"
        ]
...
class SplitConfirmationHandler(BaseStepHandler):
    ...
    async def handle_message(self, state: ConversationState, message: Message) -> StepResult:
        ...
        try:
            ...
            bill_data = BillData(**state.context["bill_data"])
            calculated_participants = [Participant(**p) for p in state.context["calculated_participants"]]
            ...
            if await self._is_confirmation_yes(content_lower):
                ...
                return StepResult(
                    ...
                    context_updates={
                        ...
                        "final_participants": [p.dict() for p in calculated_participants],
                        "confirmation_timestamp": message.timestamp.isoformat()
                    }
                )
                ...
                # Ambiguous response - ask for clarification
                current_display = await self.bill_splitter.format_split_display(bill_data, calculated_participants)
    ...
    async def _is_confirmation_yes(self, content_lower: str) -> bool:
        ...
        yes_indicators = [
            ...
            "good", "looks good", "perfect", "proceed", "go ahead", "send",
            "✓", "✅", "👍"
        ]
    ...
    async def _is_confirmation_no(self, content_lower: str) -> bool:
        ...
        no_indicators = [
            "no", "n", "nope", "wrong", "incorrect", "change", "modify", "adjust",
            "different", "recalculate", "redo", "back", "❌", "👎"
        ]
...
class PaymentTrackingHandler(BaseStepHandler):
    ...
    def __init__(self, ai_service, payment_confirmation_service: Optional[PaymentConfirmationService] = None):
        super().__init__(ai_service)
    ...
    async def handle_message(self, state: ConversationState, message: Message) -> StepResult:
        ...
        try:
            ...
            if self.payment_confirmation_service:
                ...
                if confirmation_result.success:
                    ...
                    return StepResult(
                        ...
                        next_step=ConversationStep.COMPLETED if confirmation_result.completion_detected else state.current_step,
                        context_updates={
            ...
            if self.payment_confirmation_service:
                inquiry_response = await self.payment_confirmation_service.handle_payment_inquiry(
                    sender_phone=message.metadata.get("sender_phone", ""),
                    message_content=message.content
                )
            ...
            return StepResult(
                ...
                context_updates={
                    "unrecognized_message": message.content
                }
            )
    ...
    def _is_status_request(self, message_content: str) -> bool:
        ...
        status_keywords = [
            "status", "show status", "bill status", "payment status",
            "who paid", "who hasn't paid", "check payments", "update"
        ]
        
        content_lower = message_content.lower()
        return any(keyword in content_lower for keyword in status_keywords)
    
    async def _handle_status_request(self, state: ConversationState, message: Message) -> StepResult:
...
class CompletionHandler(BaseStepHandler):
    ...
    def _is_new_bill_request(self, message_content: str) -> bool:
        ...
        new_bill_keywords = [
            "new bill", "another bill", "split another", "new split",
            "start over", "begin", "fresh start"
        ]
