...
from app.models.schemas import Message, Response, ConversationState, BillData, Participant, ErrorResponse
...
class ConversationManager(ConversationServiceInterface):
    ...
    async def process_message(self, user_id: str, message: Message) -> Response:
        ...
        try:
            ...
            return await self.error_handler.handle_conversation_error(e, user_id, message)
    
    async def get_conversation_state(self, user_id: str, session_id: str) -> ConversationState:
        """
        ...
        try:
            ...
            db_state = await self.conversation_repo.get_conversation_state(user_uuid, session_id)
    ...
    async def update_conversation_state(self, state: ConversationState) -> ConversationState:
        ...
        try:
            ...
            logger.info(f"Updated conversation state for user {state.user_id} to step {state.current_step}")
            return state
            ...
            if state.retry_count >= self.max_retry_count:
                logger.error(f"Max retries exceeded for user {state.user_id}, resetting state")
                return await self._reset_conversation_state(state.user_id, state.session_id)
            
            raise
    
    async def reset_conversation(self, user_id: str, session_id: str) -> ConversationState:
        """
    ...
    async def cleanup_expired_conversations(self) -> int:
        ...
        """
        try:
            count = await self.conversation_repo.cleanup_expired_states(self.session_timeout_hours)
    ...
    async def _check_payment_confirmation(self, message: Message) -> Optional[Response]:
        ...
        try:
            ...
            confirmation_result = await payment_service.process_payment_confirmation_message(
    ...
    async def _create_new_conversation_state(self, user_id: str, session_id: str) -> ConversationState:
        ...
        try:
            user_uuid = UUID(user_id)
            await self.conversation_repo.create_conversation_state(
                user_id=user_uuid,
                session_id=session_id,
                current_step=state.current_step.value,
                context=state.context
    ...
    async def _reset_conversation_state(self, user_id: str, session_id: str) -> ConversationState:
        """Reset conversation state to initial step"""
        try:
            # Delete existing state
            user_uuid = UUID(user_id)
            await self.conversation_repo.delete_conversation_state(user_uuid, session_id)
        except Exception as e:
    ...
    async def _validate_state(self, state: ConversationState) -> bool:
        ...
        try:
            ...
            if not await self._validate_context_for_step(state.current_step, state.context):
                return False
    ...
    async def _validate_context_for_step(self, step: ConversationStep, context: Dict[str, Any]) -> bool:
