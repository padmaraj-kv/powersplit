...
from app.services.step_handlers import (
    InitialStepHandler,
    BillExtractionHandler,
    BillConfirmationHandler,
    ContactCollectionHandler,
    SplitCalculationHandler,
    SplitConfirmationHandler,
    PaymentRequestHandler,
    PaymentTrackingHandler,
    CompletionHandler,
)
...
def get_conversation_factory() -> ConversationFactory:
    ...
    if not _conversation_factory:
        raise RuntimeError(
            "Conversation factory not initialized. Call initialize_conversation_factory first."
        )
    return _conversation_factory


def initialize_conversation_factory(
    conversation_repo: ConversationRepository,
    contact_repo: ContactRepository,
    user_repo: UserRepository,
) -> ConversationFactory:
    ...
    global _conversation_factory
    _conversation_factory = ConversationFactory(
        conversation_repo, contact_repo, user_repo
    )
