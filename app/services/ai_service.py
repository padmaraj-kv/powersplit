...
class AIService(AIServiceInterface):
    ...
    async def extract_from_text(self, text: str) -> BillData:
        ...
        try:
            bill_data = await self._retry_operation(
                self.litellm_client.extract_bill_from_text,
                text
            )
            ...
            try:
                ...
                raise AIServiceError(f"Failed to extract bill information from text: {e}")
    ...
    async def extract_from_voice(self, audio_data: bytes) -> BillData:
        ...
        try:
            transcript = await self._retry_operation(
                self.sarvam_client.transcribe_audio,
                audio_data
            )
    ...
    async def extract_from_image(self, image_data: bytes) -> BillData:
        ...
        try:
            bill_data = await self._retry_operation(
                self.gemini_client.extract_bill_from_image,
                image_data
            )
            
            # Step 3: Enhance description if possible
            try:
                enhanced_description = await self.gemini_client.enhance_bill_description(bill_data)
    ...
    async def validate_extraction(self, bill_data: BillData) -> ValidationResult:
        ...
        try:
            ...
            validation_result = await self._retry_operation(
                self.litellm_client.validate_bill_data,
                bill_data
            )
            
            logger.info(f"Validation complete: {'Valid' if validation_result.is_valid else 'Invalid'}")
    ...
    async def recognize_intent(self, message: Message, current_step: ConversationStep) -> Dict[str, Any]:
        ...
        try:
            intent_data = await self._retry_operation(
                self.litellm_client.recognize_intent,
                message.content,
                current_step
            )
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        for i, (service_name, _) in enumerate(tasks):
            ...
            if isinstance(result, Exception):
                ...
                logger.info(f"{service_name} health check: {'OK' if result else 'FAILED'}")
    ...
    def _basic_intent_recognition(self, text: str, current_step: ConversationStep) -> Dict[str, Any]:
        """
        ...
        if any(word in text_lower for word in ["yes", "ok", "correct", "right", "confirm"]):
            return {"intent": "confirm", "confidence": 0.7, "entities": {}, "next_action": "proceed"}
        elif any(word in text_lower for word in ["no", "wrong", "change", "modify"]):
            return {"intent": "modify", "confidence": 0.7, "entities": {}, "next_action": "ask_changes"}
        elif any(word in text_lower for word in ["paid", "done", "completed"]):
            return {"intent": "confirm_payment", "confidence": 0.8, "entities": {}, "next_action": "update_payment"}
        else:
            return {"intent": "general_question", "confidence": 0.5, "entities": {}, "next_action": "ask_clarification"}
