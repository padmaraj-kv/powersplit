...
class SirenClient:
    ...
    async def send_whatsapp_message(self, phone_number: str, message: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.base_url}/v1/messages/whatsapp",
                json=payload.dict()
            )
            ...
            if response.status_code == 200:
                ...
                error_msg = f"WhatsApp send failed: {response.status_code} - {response.text}"
    ...
    async def send_sms(self, phone_number: str, message: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Send SMS via Siren
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.base_url}/v1/messages/sms",
                json=payload.dict()
            )
    ...
    async def send_message_with_fallback(self, phone_number: str, message: str, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.warning(f"WhatsApp delivery failed for {phone_number}: {whatsapp_error}")
