...
class Settings(BaseSettings):
    ...
    siren_webhook_secret: str = Field(
        ..., description="Siren webhook secret for signature validation"
    )
    siren_base_url: str = Field(
        default="https://api.siren.ai",
        description="Siren API base URL",
    )

    # AI Service settings
    sarvam_api_key: str = Field(..., description="Sarvam AI API key for speech-to-text")
    gemini_api_key: str = Field(
        ..., description="Google Gemini API key for vision and text processing"
    )
    # LiteLLM uses the selected provider API key; no separate key required
    ...
    webhook_timeout: int = Field(
        default=30, description="Webhook processing timeout in seconds"
    )
    webhook_retry_attempts: int = Field(
        default=3, description="Number of webhook retry attempts"
    )

    # AI Service timeouts
    ai_service_timeout: int = Field(
        default=30, description="AI service request timeout in seconds"
    )
    ai_service_retry_attempts: int = Field(
        default=2, description="AI service retry attempts"
    )

    # Database settings
    db_pool_size: int = Field(default=10, description="Database connection pool size")
    db_max_overflow: int = Field(
        default=20, description="Database connection pool overflow"
    )
    db_pool_timeout: int = Field(default=30, description="Database connection timeout")

    # Conversation settings
    conversation_timeout_hours: int = Field(
        default=24, description="Conversation state timeout in hours"
    )
    max_conversation_retries: int = Field(
        default=3, description="Maximum conversation retry attempts"
    )

    # Payment settings
    upi_timeout_minutes: int = Field(
        default=30, description="UPI payment link timeout in minutes"
    )
    payment_reminder_hours: int = Field(
        default=24, description="Hours before sending payment reminders"
    )
...
def validate_configuration() -> bool:
    ...
    try:
        ...
        logger.info(
            f"Configuration validated successfully for environment: {settings.environment}"
        )
