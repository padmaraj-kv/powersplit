...
from app.services.conversation_factory import initialize_conversation_factory, get_conversation_factory
from app.database.repositories import SQLUserRepository, SQLContactRepository, SQLBillRepository, SQLPaymentRepository, SQLConversationRepository
...
async def register_health_checks():
    ...
    def memory_health():
        ...
        try:
            ...
            return {"memory_percent": memory.percent, "available_gb": memory.available / 1024**3}
        except ImportError:
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "bills": "/api/v1/bills/",
            "admin": "/api/v1/admin/",
            "documentation": "/docs" if not settings.is_production else "disabled_in_production"
        },
