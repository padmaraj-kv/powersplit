...
from app.database.repositories import SQLConversationRepository, DatabaseRepository
...
def verify_admin_token(x_admin_token: str = Header(None)):
    ...
    if not x_admin_token or x_admin_token != settings.jwt_secret:
...
async def get_detailed_error_report(
...
async def run_maintenance_cleanup(
    ...
    try:
        ...
        if cleanup_type in ["conversations", "all"]:
            ...
            db = get_database()
            conversation_repo = SQLConversationRepository(db)
            cleaned_conversations = await conversation_repo.cleanup_expired_states(hours=hours_back)
...
async def get_configuration_status(
    ...
    try:
        config_status = {
            ...
            "database_configured": bool(settings.supabase_url and settings.supabase_key),
            "siren_configured": bool(settings.siren_api_key and settings.siren_webhook_secret),
            "ai_services_configured": {
                "sarvam": bool(settings.sarvam_api_key),
                "gemini": bool(settings.gemini_api_key),
                "litellm": bool(settings.litellm_api_key)
            },
            "security_configured": bool(settings.encryption_key and settings.jwt_secret)
        }
