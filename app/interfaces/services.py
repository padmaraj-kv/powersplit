...
class BillExtractorInterface(ABC):
    ...
    @abstractmethod
    async def process_bill_confirmation(
        self, message: Message, bill_data: BillData
...
class BillServiceInterface(ABC):
    ...
    async def calculate_splits(
        self, bill_data: BillData, participants: List[Participant]
    ) -> List[Participant]:
...
class ContactServiceInterface(ABC):
    ...
    async def find_or_create_contact(
        self, user_id: str, name: str, phone_number: str
    ) -> str:
    ...
    async def auto_populate_from_history(
        self, user_id: str, participant_names: List[str]
    ) -> List[Participant]:
...
class PaymentServiceInterface(ABC):
    ...
    async def create_payment_message(
        self, recipient_name: str, amount: Decimal, description: str, upi_link: str
    ) -> str:
    ...
    async def create_payment_requests(
        self, bill_id: str, participants: List[Participant]
    ) -> List[PaymentRequest]:
...
class BillQueryServiceInterface(ABC):
    ...
    async def get_user_bills(
        self, user_id: str, filters: Optional["BillFilters"] = None
    ) -> List["BillSummary"]:
    ...
    async def get_bill_details(
        self, user_id: str, bill_id: str
    ) -> Optional["BillDetails"]:
        """Implements requirement 6.3 for complete bill information"""
        pass

    @abstractmethod
    async def send_payment_reminders(
        self, user_id: str, bill_id: str, participant_ids: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """Implements requirement 6.4 for resending payment requests"""
        pass

    @abstractmethod
    async def get_unpaid_participants(
        self, user_id: str, bill_id: str
